...
const getPartners = asyncHandler(async (req, res) => {
  const { page, limit, search, status, stockist_level, level } = req.query;
  const params = [];
  ...
  if (stockist_level || level) {
    where += ' AND p.stockist_level = ?';
    params.push(stockist_level || level);
  }
  ...
  try {
    ...
    if (!isMissingColumn(err, 'p.region') && !isMissingColumn(err, 'region')) {
      throw err;
    }
    ...
    const fallbackBaseQuery = `
      ...
      ${where}
      ORDER BY p.business_name ASC`;
    result = await paginate(fallbackBaseQuery, countQuery, params, page, limit);
    result.data = result.data.map((row) => ({ ...row, region: null }));
  }
...
const getPartner = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    rows = rows.map((row) => ({ ...row, region: null }));
  }
...
const createPartner = asyncHandler(async (req, res) => {
  const {
    business_name, email, phone, address, region, stockist_level,
  ...
  try {
    ...
    let partnerResult;
    try {
      [partnerResult] = await conn.execute(
        `INSERT INTO partners (business_name, email, phone, address, region, stockist_level, parent_partner_id, discount_pct)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [business_name, email, phone || null, address || null, region || null, stockist_level,
         parent_partner_id || null, discount_pct !== undefined ? discount_pct : 0]
      );
    } catch (err) {
      if (!isMissingColumn(err, 'region')) throw err;
      [partnerResult] = await conn.execute(
...
const updatePartner = asyncHandler(async (req, res) => {
  ...
  const { business_name, email, phone, address, region, status, parent_partner_id, stockist_level, discount_pct } = req.body;
  ...
  if (stockist_level !== undefined) { fields.push('stockist_level = ?'); values.push(stockist_level); }
  if (discount_pct !== undefined) { fields.push('discount_pct = ?'); values.push(discount_pct); }
  ...
  try {
    ...
    const filtered = fields
      ...
      .filter((entry) => entry.field !== 'region = ?');
    if (filtered.length === 0) throw ApiError.badRequest('No fields to update');
    await pool.execute(
      `UPDATE partners SET ${filtered.map((entry) => entry.field).join(', ')} WHERE id = ?`,
      [...filtered.map((entry) => entry.value), partnerId]
    );
  }
